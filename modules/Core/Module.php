...
class Module extends Autoload
{
     ...
     * @return $this
     */
    public function ADDM(\Module $module, $pos)
    {
        ...
        $pos = str_replace('$', '', $pos);
        $posv = $pos;
        if (isset($module->__cms_module_position) && !empty($module->__cms_module_position) && ($pos === self::NO_POSITION || empty($pos)))
        {
            $posv = $module->__cms_module_position;
        }

        if (!empty($posv))
        {
            $module->__cms_module_position = $posv;
        }
        else if ($pos === self::NO_POSITION || empty($pos))
        {
            $module->not_render();
        }
     ...
     * @return $this
     */
    public function clone_settings($module, $filter = array(), $takeEmpty = true)
    {
        routes::not_ajax(__METHOD__);
